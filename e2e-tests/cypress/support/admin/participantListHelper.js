...
export function assertParticipantListLength(expectedLength) {
  return getParticipantRows().should('have.length', expectedLength);
}
...
export function selectParticipantOnWaitingList(index) {
  getByTestId('waitinglist-participants').within(() => {
    getParticipantRows().eq(index).click({ force: true });
  });
}

export function openDeleteParticipantDialog() {
  getByTestId('delete-participant-dialog-action').click({ force: true });
}
...
export function assertParticipantListInfoWithText(expectedTextToBeContained) {
  return getByTestId('participant-list-info-box').should('contain', expectedTextToBeContained);
}
...
export function getParticipantListSearchTextInput() {
  return getByTestId('participant-list-search-input').find('input').first();
}
