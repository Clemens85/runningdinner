...
export const TeamMemberCancelDialog = ({adminId, team, teamMemberToCancel, isOpen, onClose}) => {
  ...
  const teamMemberToCancelFullname = ParticipantService.getFullname(teamMemberToCancel);

  const { cancelWholeTeam, remainingTeamMemberNames } = TeamService.getTeamMemberCancelInfo(team, teamMemberToCancel);

  const {enqueueSnackbar} = useSnackbar();

  const cancelTeamMemberAsync = async() => {
    try  {
      const updatedTeam = await TeamService.cancelTeamMemberAsync(adminId, team.id, teamMemberToCancel.id);
