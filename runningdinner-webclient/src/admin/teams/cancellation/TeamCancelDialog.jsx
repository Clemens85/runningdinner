...
export const TeamCancelDialog = ({runningDinner, teamToCancel, isOpen, onClose}) => {
  ...
  const cancelTeamAsync = async(replacementParticipants) => {
    try {
      const teamCancellationResult = await TeamService.cancelTeamAsync(adminId, teamToCancel, replacementParticipants);
  ...
  const handleShowPreview = async() => {
    try {
      const teamCancellationPreviewResponse = await TeamService.cancelTeamDryRunAsync(adminId, teamToCancel, replacementParticipants);
  ...
  return (
      <Dialog open={true} onClose={onCancelDialog} aria-labelledby="form-dialog-title" maxWidth={"sm"} fullWidth={true}>
        ...
        <DialogContent>
          ...
          <Box mt={3}>
            <Fetch asyncFunction={ParticipantService.findNotAssignedParticipantsAsync}
                   parameters={[adminId]}
