...
const handleTeamPartnerWishAction = async (adminId, teamPartnerWishAction) => {
  ...
  switch (type) {
    case CREATE_NEW_PARTICIPANT_TEAM_PARTNER_WISH_ACTION:
      resultPayload = ParticipantService.newEmptyParticipantInstance();
    ...
    case SEND_INVITATION_TEAM_PARTNER_WISH_ACTION:
      resultPayload = await MessageService.sendTeamPartnerWishInvitationAsync(adminId, fromParticipant);
      break;
    case UPDATE_PARTICIPANT_TEAM_PARTNER_WISH_ACTION:
      ...
      resultPayload = await ParticipantService.saveParticipantAsync(adminId, matchingParticipant);
      break;
    default:
