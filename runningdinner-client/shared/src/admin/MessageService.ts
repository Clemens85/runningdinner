...
import { isArrayEmpty, isStringEmpty, isStringNotEmpty, removeEntitiesFromList } from '../Utils';
...
export async function sendMessagesAsync<T extends BaseMessage>(
  ...
  mailProviderToUse?: string | null,
): Promise<MessageJob> {
  const sendToDinnerOwnerStr = sendToDinnerOwner === true ? 'true' : 'false';
  const recipientType = mapRecipientType(messageType);
  let url = BackendConfig.buildUrl(`/messageservice/v1/runningdinner/${adminId}/mails/${recipientType}?sendToDinnerOwner=${sendToDinnerOwnerStr}`);
  if (isStringNotEmpty(mailProviderToUse)) {
    url += `&mailProviderToUse=${mailProviderToUse}`;
  }
...
export function getStatusResult(messageJobOrTask: MessageJob | MessageTask): string {
  ...
  if (sendingFailed === null) {
    const deliveryFailed = get(messageJobOrTask, 'sendingResult.delieveryFailed', null);
    if (deliveryFailed === true) {
      sendingFailed = FuzzyBoolean.TRUE;
    } else {
      sendingFailed = FuzzyBoolean.FALSE;
    }
  }

  if (sendingFailed === FuzzyBoolean.TRUE) {
...
export function getEffectiveSelectedRecipients(allRecipients: Recipient[], recipientSelection: string, customSelectedRecipients?: Recipient[]): Recipient[] {
