...
export async function findParticipantRecipients(
  adminId: string
): Promise<ParticipantListable[]> {
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/participants`
  );
  const response = await axios.get(url);
  return response.data;
}

export async function findMessageJobsByAdminIdAndTypeAsync(
  adminId: string,
  messageType: MessageType
): Promise<MessageJob[]> {
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/messagejobs?messageType=${messageType}`
  );
  const response = await axios.get(url);
  return response.data;
}

export async function findMessageJobOverviewByAdminIdAndMessageJobId(
  adminId: string,
  messageJobId: string
): Promise<MessageJobOverview> {
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/messagejobs/${messageJobId}/overview`
  );
  const response = await axios.get(url);
  return response.data;
}

export async function findMessageJobByAdminIdAndJobIdAsync(
  adminId: string,
  messageJobId: string
): Promise<MessageJob> {
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/messagejobs/${messageJobId}`
  );
  const response = await axios.get(url);
  return response.data;
}

export async function findMessageTasksByAdminIdAndJobIdAsync(
  adminId: string,
  messageJobId: string
): Promise<MessageTask[]> {
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/messagetasks/${messageJobId}`
  );
  const response = await axios.get(url);
  return response.data;
}

export async function sendMessagesAsync<T extends BaseMessage>(
  adminId: string,
  messageObject: T,
  messageType: MessageType,
  sendToDinnerOwner: boolean
): Promise<MessageJob> {
  const sendToDinnerOwnerStr = sendToDinnerOwner === true ? "true" : "false";
  const recipientType = mapRecipientType(messageType);
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/mails/${recipientType}?sendToDinnerOwner=${sendToDinnerOwnerStr}`
  );
...
export async function getMessagePreviewAsync<T extends BaseMessage>(
  adminId: string,
  messageTemplate: T,
  recipientToUse: BaseEntity,
  messageType: MessageType
): Promise<PreviewMessageList> {
  const recipientType = mapRecipientType(messageType);
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/mails/${recipientType}/preview`
  );
  const messageObject = getMailMessageForSelectedRecipient(
    messageTemplate,
    messageType,
    recipientToUse
  );
...
export async function sendTeamPartnerWishInvitationAsync(
  adminId: string,
  participant: Participant
): Promise<MessageJob> {
  const { id } = participant;
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/mails/teampartnerwish/${id}`
  );
  const response = await axios.put(url);
  return response.data;
}

export async function reSendMessageTaskAsync(
  adminId: string,
  messageTask: MessageTask
) {
  const { id } = messageTask;
  const url = BackendConfig.buildUrl(
    `/messageservice/v1/runningdinner/${adminId}/messagetask/${id}`
  );
  const response = await axios.put(url, messageTask);
  return response.data;
}

export function getMailMessageForSelectedRecipient<T extends BaseMessage>(
  mailMessageTemplate: T,
  messageType: MessageType,
  recipientForPreview: BaseEntity
) {
  ...
  if (recipientForPreview) {
    ...
    if (
      ...
      (mailMessage as BaseTeamMessage).teamSelection =
        TeamSelection.CUSTOM_SELECTION;
      // @ts-ignore
      (mailMessage as BaseTeamMessage).customSelectedTeamIds = [
        recipientForPreview?.id,
      ];
    } else {
      (mailMessage as ParticipantMessage).participantSelection =
        ParticipantSelection.CUSTOM_SELECTION;
      // @ts-ignore
      (mailMessage as ParticipantMessage).customSelectedParticipantIds = [
        recipientForPreview?.id,
      ];
    }
  }
...
export function getStatusResult(
  messageJobOrTask: MessageJob | MessageTask
): string {
...
export function getExampleTeamMessage(): TeamMessage {
  return {
    ...
    hostMessagePartTemplate:
      "Es wird vorgeschlagen, dass du als Gastgeber fungierst. Wenn dies nicht in Ordnung ist, dann sprecht euch bitte ab und gebt uns Rückmeldung wer als neuer Gastgeber fungieren soll.",
...
function mapEmptySelectionStringToNull<T extends BaseMessage>(
  incomingMailMessage: T
): T {
  const result = cloneDeep(incomingMailMessage);
  if (
    (result as any).participantSelection &&
    (result as any).participantSelection === ""
  ) {
...
export function getEffectiveSelectedRecipients(
  ...
  return calculateRecipientsForSpecialSelection(
    allRecipients as Participant[],
    recipientSelection
  );
}

function calculateRecipientsForSpecialSelection(
  allParticipants: Participant[],
  participantSelection: string
): Recipient[] {
  const assignedParticipants =
    filterParticipantsOrganizedInTeams(allParticipants);
  if (
    participantSelection === CONSTANTS.PARTICIPANT_SELECTION.ASSIGNED_TO_TEAM
  ) {
    return assignedParticipants;
  } else if (
    participantSelection ===
    CONSTANTS.PARTICIPANT_SELECTION.NOT_ASSIGNED_TO_TEAM
  ) {
