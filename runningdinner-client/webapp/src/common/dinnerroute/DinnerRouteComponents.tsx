...
export function getTeamLabel(team: DinnerRouteTeam, includeHostFullname: boolean) {
  if (includeHostFullname && team.hostTeamMember) {
    return (
      <>
        Team #{team.teamNumber} ({team.meal.label}) - <Fullname {...team.hostTeamMember} />
      </>
    );
  } else {
    return (
      <>
        Team #{team.teamNumber} ({team.meal.label})
      </>
    );
  }
}
    ...
    return (
      <>
        {contactInfoUnique.map((mobileNumber: string, index: number) => (
          <React.Fragment key={mobileNumber}>
            {index > 0 ? <span>, &nbsp;</span> : ''}
            <LinkExtern href={`tel:${mobileNumber}`} title={mobileNumber} />
          </React.Fragment>
        ))}
      </>
    );
          ...
          <Alert severity="warning" variant="outlined" onClose={close}>
            ...
            <ul>
              {teamsWithUnresolvedGeocodings.map((team) => (
                <li key={team.teamNumber}>{getTeamLabel(team, true)}</li>
              ))}
            </ul>
          </Alert>
        </Box>
...
export function AfterPartyLocationCard(afterPartyLocation: AfterPartyLocation) {
  return (
    <Span>
      ...
      <>
        {isStringNotEmpty(afterPartyLocation.addressRemarks) && (
          <>
            <br />
            {afterPartyLocation.addressRemarks}
          </>
        )}
      </>
    </Span>
