...
export function ZipRestrictionsFormControl({ currentRegistrationType, currentZipRestrictions, onUpdateZipRestrictions }: ZipRestrictionsFormControlProps) {
  ...
  return (
    <>
          ...
          <Box>
            {hasZipRestrictions && (
              <Typography variant="body2">
                {t('common:zip_restrictions_enabled')} <strong>{currentZipRestrictions}</strong>
              </Typography>
            )}
            {!hasZipRestrictions && <Typography variant="body2">{t('common:zip_restrictions_disabled')}</Typography>}
          </Box>
          <Box>
            <SecondaryButton color={'primary'} variant={'outlined'} size={'small'} onClick={handleOpenEditZipRestrictionsDialog}>
              {buttonLabel}
            </SecondaryButton>
          </Box>
        </Stack>
      ...
      {isOpen && (
        <EditZipRestrictionsDialog
          currentZipRestrictions={isStringNotEmpty(currentZipRestrictions) ? currentZipRestrictions : ''}
...
function EditZipRestrictionsDialog({ onCancel, onSave, currentZipRestrictions }: EditZipRestrictionsDialogProps) {
  ...
  return (
    <Dialog open={true} onClose={onCancel} aria-labelledby={title}>
      ...
      <DialogActionsPanel onOk={handleSave} onCancel={onCancel} okLabel={t('common:apply')} cancelLabel={t('common:cancel')} />
    </Dialog>
  );
}
