...
import { CallbackHandler, Feedback, FeedbackData, HttpError, newEmptyFeedbackInstance, newUuid, useBackendIssueHandler } from '@runningdinner/shared';
...
export function FeedbackDialog({ onClose }: FeedbackDialogProps) {
  ...
  // const [threadId, setThreadId] = useState(newUuid());
  ...
  return (
    <Dialog onClose={onClose} open={true} maxWidth="md">
      ...
      <DialogActionsPanel onOk={handleSubmit(handleSubmitFeedback)} onCancel={onClose} okLabel={t('common:save')} cancelLabel={t('common:cancel')} />
    </Dialog>
  );
}
