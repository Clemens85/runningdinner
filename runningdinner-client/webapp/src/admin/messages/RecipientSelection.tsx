...
function RecipientSelection({ messageType }: MessageTypeAdminIdPayload) {
  ...
  const handleFinishEditCustomSelectedRecipients = (updatedCustomSelectedRecipients: Recipient[]) => {
    ...
    if (isArrayEmpty(updatedCustomSelectedRecipients)) {
      ...
      const updatedSelectedRecipients = getEffectiveSelectedRecipients(
        recipients.data || [],
        CONSTANTS.RECIPIENT_SELECTION_COMMON.CUSTOM_SELECTION,
        updatedCustomSelectedRecipients,
      );
      changeRecipientForPreviewIfNeeded(updatedSelectedRecipients);
    }
  };
  ...
  return (
    <Box mb={3} mt={2}>
      ...
      <SelectionHelperText
        currentSelection={currentRecipientSelectionValue}
      ...
      {showCustomSelectionDialog && (
        <SingleSelectionDialog
          open={showCustomSelectionDialog}
...
function SelectionHelperText({ messageType, currentSelection, customSelectedEntities, onEditCustomSelectedEntities, numberOfSelectedRecipients }: SelectionHelperTextProps) {
  ...
  if (isArrayNotEmpty(customSelectedEntities)) {
    const customSelectEntitiesList = customSelectedEntities.map((entity) => <SmallTitle key={entity.id}>{getRecipientName(entity)}</SmallTitle>);
    ...
    return (
      <>
        <Typography variant={'subtitle2'}>
          {label}
          <IconButton onClick={onEditCustomSelectedEntities} aria-label={t('common:change')} size="large">
            <EditIcon />
          </IconButton>
        </Typography>
  ...
  if (messageType === MessageType.MESSAGE_TYPE_PARTICIPANTS) {
    if (currentSelection === CONSTANTS.PARTICIPANT_SELECTION.ASSIGNED_TO_TEAM) {
      return (
        <Box sx={{ mt: 1 }}>
          <Span html={true} i18n="admin:participant_selection_assigned_to_teams_text" parameters={{ numberOfSelectedParticipants: numberOfSelectedRecipients }} />
        </Box>
      );
    } else if (currentSelection === CONSTANTS.PARTICIPANT_SELECTION.NOT_ASSIGNED_TO_TEAM) {
      return (
        <Box sx={{ mt: 1 }}>
          <Span html={true} i18n="admin:participant_selection_not_assigned_to_teams_text" parameters={{ numberOfSelectedParticipants: numberOfSelectedRecipients }} />
        </Box>
      );
    } else {
      return (
        <Box sx={{ mt: 1 }}>
          <Span html={true} i18n="admin:participant_selection_all_text" parameters={{ numberOfSelectedParticipants: numberOfSelectedRecipients }} />
        </Box>
      );
    }
  } else {
    return (
      <Box sx={{ mt: 1 }}>
        <Span html={true} i18n="admin:team_selection_all_text" parameters={{ numTeams: numberOfSelectedRecipients }} />
      </Box>
    );
  }
}
