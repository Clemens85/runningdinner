...
export function TeamArrangementActionsButton({ adminId }: TeamArrangementActionsButtonProps) {
  ...
  const {
    isOpen: showRegenerateTeamsConfirmation,
    open: openRegenerateTeamsConfirmation,
    getIsOpenData: getRegenerateTeamsConfirmationData,
    close: closeRegenerateTeamsConfirmation,
  } = useDisclosure<TeamArrangementActionConfirmationData>();

  const {
    isOpen: showDropTeamsConfirmation,
    open: openDropTeamsConfirmation,
    getIsOpenData: getDropTeamsConfirmationData,
    close: closeDropTeamsConfirmation,
  } = useDisclosure<TeamArrangementActionConfirmationData>();
  ...
  return (
    <>
      <DropdownButton label={t('admin:teams_arrangement_action_button')} color="secondary" fullWidth={isMobileDevice}>
        <DropdownButtonItem onClick={() => handleOpenTeamArrangementActionConfirmationDialog(TEAM_ARRANGEMENT_ACTION.REGENERATE)}>{t('admin:teams_reset')}...</DropdownButtonItem>
        <DropdownButtonItem onClick={() => handleOpenTeamArrangementActionConfirmationDialog(TEAM_ARRANGEMENT_ACTION.DROP)}>{t('admin:teams_drop')}...</DropdownButtonItem>
      </DropdownButton>
      {showRegenerateTeamsConfirmation && (
        <ConfirmationDialog
          onClose={handleRegenerateTeamsConfirmationDialogClosed}
      ...
      {showDropTeamsConfirmation && (
        <ConfirmationDialog
          onClose={handleDropTeamsConfirmationDialogClosed}
