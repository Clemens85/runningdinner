...
const buildScheduledMealsWithTeams = (teamMeetingPlan) => {
  ...
  let result = teamMeetingPlan.hostTeams.map((hostTeam) => {
    return { meal: hostTeam.meal, hostTeam: hostTeam, current: false, guestTeams: hostTeam.meetedTeams.concat(activeTeam) };
  });
...
function MeetedTeamButton({ team, adminId }) {
  ...
  const teamMemberNodes = isCancelled
    ? null
  ...
  return (
    <>
      ...
      {isCancelled && (
        <Button variant="contained" color="secondary" size="medium" {...bindTrigger(popupState)}>
          <TeamNr {...team} />
        </Button>
      )}
      <Popover
