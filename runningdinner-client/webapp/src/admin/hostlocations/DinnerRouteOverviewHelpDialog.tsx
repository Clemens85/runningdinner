...
export function DinnerRouteOverviewHelpDialog({ onClose }: DinnerRouteOverviewHelpDialogProps) {
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Paragraph>
          {t('admin:dinner_route_help_intro_1')}
          <br />
          {t('admin:dinner_route_help_intro_2')}
        </Paragraph>

        <Divider sx={dividerSxProps}>
          <strong>{t('common:general')}</strong>
        </Divider>
        ...
        <br />
        <Paragraph>{t('admin:dinner_route_help_meals')}</Paragraph>
        <List dense>
          {Object.keys(mealTypeMappings).map((mealId) => (
            <ListItem disablePadding key={mealId}>
              <ListItemButton>
                <ListItemIcon>{getMealTypeIcon(mealTypeMappings[mealId])}</ListItemIcon>
          ...
          {isAfterPartyLocationDefined(afterPartyLocation) && (
            <ListItem disablePadding>
              <ListItemButton>
                <ListItemIcon>{getAfterPartyLocationIcon()}</ListItemIcon>
        ...
        <Divider sx={dividerSxProps}>
          <strong>{t('admin:dinner_route_help_near_distance_title')}</strong>
        </Divider>
        <Paragraph>{t('admin:dinner_route_help_near_distance')}</Paragraph>

        <Divider sx={dividerSxProps}>
          <strong>{t('admin:dinner_route_filter_meal_routes_title')}</strong>
        </Divider>
        <Paragraph>
          {t('admin:dinner_route_help_from_meal_to_meal_1')}
          <br />
          <Trans i18nKey={'admin:dinner_route_help_from_meal_to_meal_2'} values={{ from: meals[0].label, to: meals[1].label }} components={{ strong: <strong /> }} />
          <br />
          <Trans i18nKey={'admin:dinner_route_help_from_meal_to_meal_3'} values={{ from: meals[0].label, to: meals[1].label }} components={{ strong: <strong /> }} />
          <br />
          <Trans i18nKey={'admin:dinner_route_help_from_meal_to_meal_4'} components={{ strong: <strong /> }} />
        </Paragraph>

        <Divider sx={dividerSxProps}>
          <strong>{t('admin:dinner_route_help_distances')}</strong>
        </Divider>
        <Typography variant={'body1'} component="div">
          {t('admin:dinner_route_help_distances_1')}
          <br />
          {t('admin:dinner_route_help_distances_2')}
          <br />
          ...
          <br />
          {t('admin:dinner_route_help_distances_5')} <Chip label={`Team 1`} color={'primary'} variant={'filled'} sx={{ mr: 1, ml: 1 }} />
          <br />
        </Typography>

        <Divider sx={dividerSxProps}>
          <strong>{t('common:misc')}</strong>
        </Divider>
        <Paragraph>
          <Trans i18nKey={'admin:dinner_route_help_misc'} />
        </Paragraph>
      </DialogContent>
      <DialogActionsPanel onOk={handleClose} onCancel={handleClose} okLabel={t('common:ok')} cancelLabel={t('common:cancel')} />
    </Dialog>
  );
}
