...
import {
  ...
  swapParticipantNumbersAsync,
  useBackendIssueHandler,
  useFindParticipantsListMandatory,
...
function SelectParticiantToSwitch({ adminId, srcParticipant, selectedParticipant, onSelectedParticipantChange }: SelectParticiantToSwitchProps) {
  ...
  const participantOptions = useMemo(() => getParticipantSelectionOptions(srcParticipant, participantList), [participantList, srcParticipant]);
    ...
    <>
      ...
      <Autocomplete
        ...
        onChange={(_evt, newValue) => {
          onSelectedParticipantChange(newValue);
        }}
...
export function SwapParticipantNumbersDialog({ onCancel, onParticipantsSwapped, srcParticipant, adminId }: SwitchParticipantNumbersDialogProps) {
  ...
  const okButton = (
    <PrimarySuccessButtonAsync disabled={hasTeam || !selectedParticipant} onClick={handleSwapParticipantNumbers}>
    ...
    <Dialog open={true} onClose={onCancel} fullWidth={true}>
      ...
      <DialogContent>
        <Box>
          ...
          {!hasTeam && (
            <SelectParticiantToSwitch
              srcParticipant={srcParticipant}
              adminId={adminId}
      ...
      <DialogActions>
        <DialogActionsButtons okButton={okButton} cancelButton={<DefaultDialogCancelButton onCancel={onCancel} cancelLabel={t('common:cancel')} />} />
      </DialogActions>
    </Dialog>
  );
}
