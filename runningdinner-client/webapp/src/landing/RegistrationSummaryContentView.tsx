  ...
  return (
    <>
      ...
      <Box mb={2}>
        ...
        {isStringNotEmpty(registrationSummary.addressRemarks) && (
          <Span>
            <em>{registrationSummary.addressRemarks}</em>
          </Span>
        )}
      </Box>
      <Box mb={2}>
      ...
      <Box mb={2}>
        {registrationSummary.canHost && (
          <div style={{ display: 'flex' }}>
            ...
            <Span>
              &nbsp; {t('landing:registration_can_host')} ({t('common:participant_seats', { numSeats: registrationSummary.numberOfSeats })})
            </Span>
          </div>
        )}
        {showNotEnoughSeatsMessage && (
          <Span>
            {t('landing:registration_no_host')} ({t('common:participant_seats', { numSeats: registrationSummary.numberOfSeats })})
          </Span>
        )}
      </Box>
      ...
      <Box mb={2}>
        ...
        {registrationSummary.ageSpecified && (
          <Span>
            {t('common:age')}: {registrationSummary.age}
          </Span>
        )}
        {!registrationSummary.ageSpecified && (
          <Span>
            {t('common:age')}: {t('landing:gender_unknown')}
          </Span>
        )}
        {isStringNotEmpty(registrationSummary.notes) && (
          <Span>
            <em>{registrationSummary.notes}</em>
          </Span>
        )}
        ...
        {isStringNotEmpty(mealSpecificsAsString) && registrationSummary.mealSpecifics && (
          <Box>
            <Span>
              {t('mealspecifics_summary_text')}: {mealSpecificsAsString}
            </Span>
            {isStringNotEmpty(registrationSummary.mealSpecifics.mealSpecificsNote) && (
              <Span>
                <em>{registrationSummary.mealSpecifics.mealSpecificsNote}</em>
              </Span>
            )}
          </Box>
        )}
      ...
      {isStringNotEmpty(registrationSummary.teamPartnerWishEmail) && (
        <Box mb={2}>
          ...
          {registrationSummary.teamPartnerWishState === TeamPartnerWishState.EXISTS_SAME_TEAM_PARTNER_WISH && (
            <Span noWrap={true}>
              <Trans i18nKey={'landing:teampartner_wish_summary_match'} components={{ italic: <em /> }} values={{ teamPartnerWish: registrationSummary.teamPartnerWishEmail }} />
            </Span>
          )}
          {registrationSummary.teamPartnerWishState === TeamPartnerWishState.NOT_EXISTING && (
            <Span noWrap={true}>
              <Trans
                i18nKey={'landing:teampartner_wish_summary_not_existing'}
        ...
        <Box mb={2}>
          <Span noWrap={true}>
            <Trans
              ...
              components={{ italic: <em /> }}
              values={{ firstname: registrationSummary.teamPartnerWishRegistrationData?.firstnamePart, lastname: registrationSummary.teamPartnerWishRegistrationData?.lastname }}
            />
          </Span>
        </Box>
      )}
