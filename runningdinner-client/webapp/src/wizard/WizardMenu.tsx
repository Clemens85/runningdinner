...
export default function WizardMenu() {
  ...
  React.useEffect(() => {
    // Disable back button after dinner is created in wizard
      ...
      <div>
        ...
        <WizardToolbar>
          <Typography component="h2" variant="h5" color="inherit" align="center" noWrap>
            Run Your Dinner Wizard
          </Typography>
        </WizardToolbar>
        <WizardAppBar position="static">
          <WizardToolbar>{isStringEmpty(administrationUrl) && <NavigationLinkList />}</WizardToolbar>
          <WizardProgressBar />
        </WizardAppBar>
      </div>
      <div>
...
function NavigationLinkList() {
  ...
  React.useEffect(() => {
    // Ensure that jumping to a step is only possible if all previous steps are run through:
    ...
    <NavList aria-labelledby="main navigation">
      {navigationSteps.map(({ label, value }) => (
        <NavListItem key={value}>
          <NavListItemIcon>{navigationStepIconMap[value]}</NavListItemIcon>
          {!isSmallDevice && (
            <>
...
function WizardProgressBar() {
  ...
  return <>{showLoadingProgress ? <LinearProgress color="secondary" /> : <LinearProgress variant="determinate" value={percentage} />}</>;
}
