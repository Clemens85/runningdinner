...
public class DinnerRouteMessageFormatterTest {
  ...
  public void mobileNumbersFormattedForBothTeamMembers() {
    ...
    String dinnerRouteMessage = formatter.formatDinnerRouteMessage(runningDinner, team.getHostTeamMember(), team,
        dinnerRoute, dinnerRouteMessageTemplate);
    assertThat(dinnerRouteMessage).contains("Kontakt: 123456789, 987654321");
    assertThat(dinnerRouteMessage).contains("Kontakt: N/A"); // The other team in dinner-route have no mobile numbers
                                                             // setup, hence we get this one always also
  }
  ...
  public void mobileNumbersFormattedForPartnerTeamMemberWhenHostHasNone() {
    ...
    private List<Team> generateTeams(RunningDinner runningDinner) {
      ...
      try {
        ...
        GeneratedTeamsResult teamsResult = runningDinnerCalculator.generateTeams(runningDinnerConfig, teamMembers,
            Collections.emptyList(), Collections::shuffle);
        runningDinnerCalculator.assignRandomMealClasses(teamsResult, runningDinnerConfig.getMealClasses(),
    ...
    private void setupLocalizationProviderServiceMock() {
      ...
      Mockito.when(localizationProviderService.getTimeFormatter())
          .thenReturn(DateTimeUtil.getTimeFormatter(GERMAN.getLanguage()));
    }
  }
