...
- [X] T007 [P] Create backend DTOs in `runningdinner-backend/src/main/java/org/runningdinner/portal/`: `PortalCredentialTO` (internal helper — role, selfAdminId, participantId, adminId; **not part of the REST API contract**), `PortalConfirmRequestTO` (confirmPublicDinnerId, confirmParticipantId, confirmAdminId — POST /token/{t}/confirm body), `PortalMyEventsRequestTO` (portalToken string — POST /my-events body), `PortalEventEntryTO` (eventName, eventDate, city, role, adminUrl) — per data-model.md
- [X] T008 [P] Create `PortalTokenProvider` interface in `runningdinner-backend/src/main/java/org/runningdinner/mail/PortalTokenProvider.java` with single method `String getOrCreatePortalToken(String email)` — this is the only interface core email formatters will depend on (plan.md constitution check)
- [X] T009 [P] Create `PortalTypes.ts` in `runningdinner-client/shared/src/portal/PortalTypes.ts` with `PortalRole`, `PortalEventEntry`, `PortalMyEventsResponseTO` — per data-model.md TypeScript types section. **No credential types** (`PortalCredential`, `ParticipantPortalCredential`, `OrganizerPortalCredential`) — the frontend never holds individual event credentials
- [X] T010 Create `PortalStorageService.ts` in `runningdinner-client/shared/src/portal/PortalStorageService.ts` implementing `getStoredPortalToken()`, `storePortalToken(token)`, `clearStoredPortalToken()` using `localStorage` key `runningdinner_portal_token`; create `PortalStorageService.test.ts` alongside it covering store, retrieve, overwrite, and clear
...
- [X] T012 [US1] Implement `validatePortalToken(String portalToken)` and `performEventConfirmation(String portalToken, String confirmPublicDinnerId, UUID confirmParticipantId, String confirmAdminId)` in `ParticipantPortalService.java`. `validatePortalToken` is side-effect-free (read-only, 404 if missing). `performEventConfirmation` validates the token and triggers the appropriate idempotent confirmation. Separated so the GET endpoint has no side effects (prevents email scanner bot confirmation).
- [X] T013 [US1] Create `ParticipantPortalServiceRest.java` in `runningdinner-backend/src/main/java/org/runningdinner/portal/ParticipantPortalServiceRest.java`; map `GET /rest/participant-portal/v1/token/{portalToken}` → `validatePortalToken()` (204, no params, no side effects) and `POST /rest/participant-portal/v1/token/{portalToken}/confirm` → `performEventConfirmation()` (204, body `PortalConfirmRequestTO`) per contracts/rest-api.md
...
- [X] T015 [P] [US1] Create `PortalService.ts` in `runningdinner-client/shared/src/portal/PortalService.ts`; implement `validatePortalToken(portalToken)` calling `GET /rest/participant-portal/v1/token/{portalToken}` (returns void, 204) and `confirmPortalEvent(portalToken, params)` calling `POST /rest/participant-portal/v1/token/{portalToken}/confirm` (returns void, 204) via `BackendConfig.buildUrl()`
- [X] T016 [US1] Create `PortalActivationPage.tsx` in `runningdinner-client/webapp/src/portal/PortalActivationPage.tsx`: extract `portalToken` from path params and confirmation query params; call `validatePortalToken()` (GET, no side effects); if confirmation params are present call `confirmPortalEvent()` (POST); on success call `storePortalToken(portalToken)` then navigate to `/my-events`; show loading indicator during fetch; show generic error on 404
- [X] T017 [US1] Create `runningdinner-client/webapp/src/portal/PortalApp.tsx` as the portal module entry point with its own React Router `<Routes>` block; register route `/my-events/:portalToken` → `PortalActivationPage` (additional routes added in T024)
- [X] T018 [US1] Create `ParticipantPortalServiceTest.java` in `runningdinner-backend/src/test/java/org/runningdinner/portal/ParticipantPortalServiceTest.java` using `@ApplicationTest`; cover: token creation idempotency, `validatePortalToken` succeeds after registration, `performEventConfirmation` with valid participant confirmation, `performEventConfirmation` idempotent re-confirm, `resolveMyEvents` with valid token, 404 on unknown token
...
- [X] T019 [US2] Implement `resolveMyEvents(PortalMyEventsRequestTO request)` in `ParticipantPortalService.java`: look up `PortalToken` by `request.getPortalToken()` (404 if missing); resolve all events for the bound email: participant events via `participantService.findParticipantsAcrossAllDinnersByEmail` + `runningDinnerService`; organizer events via `runningDinnerService.findRunningDinnersByOrganizerEmail`; silently skip unresolvable entries; return `PortalMyEventsResponseTO` with list of `PortalEventEntryTO`
- [X] T020 [US2] Add `POST /rest/participant-portal/v1/my-events` endpoint to `ParticipantPortalServiceRest.java`; accept `PortalMyEventsRequestTO` (body: `{ portalToken }`); delegate to `ParticipantPortalService.resolveMyEvents()`; return `200 OK` per contracts/rest-api.md
- [X] T021 [P] [US2] Add `fetchMyEvents(portalToken: string)` to `PortalService.ts` calling `POST /rest/participant-portal/v1/my-events` with body `{ portalToken }`; create `useMyEvents.ts` in `runningdinner-client/shared/src/portal/useMyEvents.ts` React Query hook wrapping `fetchMyEvents` with token from `getStoredPortalToken()`
...
- [X] T024 [US2] Register route `/my-events` → `MyEventsPage` in `PortalApp.tsx` (same file as T017)
- [X] T025 [US2] Export `PortalEventEntry`, `PortalRole`, `PortalStorageService` functions, `PortalService` functions, `useMyEvents` from `runningdinner-client/shared/src/index.ts`
...
- [X] T029 [P] [US3] Use the shared navbar component (extracted in T027) inside `PortalApp.tsx` so the portal renders the same top-level navigation as the landing page — including the newly added "My Events" link
...
- [X] T035 [US4] Integrate `AccessRecoveryForm` into `MyEventsPage.tsx` empty state — render form inline when event list is empty, per FR-006 (no separate page navigation)
...
- [X] T037 [US5] Implement `confirmAdminId` handling in `ParticipantPortalService.performEventConfirmation()`: if `confirmAdminId` is present, look up the `RunningDinner` via `RunningDinnerService`, set `acknowledgedDate` to the current date/time if currently null (idempotent); organizer event resolution in `resolveMyEvents` uses the email-based lookup — no credential-level adminId is submitted by the frontend
...
- [X] T040 [P] [US5] Handle `?confirmAdminId` query param in `runningdinner-client/webapp/src/portal/PortalActivationPage.tsx` — read it from `searchParams` and pass it to `confirmPortalEvent()` call (separate POST, not the GET) so organizer confirmation is not triggered by email scanner prefetch
...
**Independent Test**: Store events → trigger "Forget me" action → confirm dialog → portal shows empty state → check `localStorage` — key `runningdinner_portal_token` is absent.

- [X] T042 [P] [US6] Create `ForgetMeButton.tsx` in `runningdinner-client/webapp/src/portal/ForgetMeButton.tsx`: button that opens a MUI confirmation dialog; on confirm calls `clearStoredPortalToken()` from `PortalStorageService` then navigates to `/my-events` (empty state will render naturally)
...
- [X] T045 Add backward-compatibility route in the landing module (`runningdinner-client/webapp/src/landing/`): handle old URL `/running-dinner-events/:publicDinnerId/:participantId/activate` — perform existing activation logic then redirect to `/my-events` so the user lands in the portal; per quickstart.md Flow E
...
- [X] T047 Run quickstart.md Flows A–E manually against the integrated system and fix any deviations; update quickstart.md if any URLs or steps changed during implementation
